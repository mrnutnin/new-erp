...
- UX action order is consistent across workflows: primary action (approve/post/save) first, destructive cancel/reversal second, and back/list last; use semantic button colors (`primary`/`success`/`dark` for forward actions, `outline-danger` for destructive actions, `outline-dark`/`outline-secondary` for navigation). Any page with filters keeps them in a separate card and provides a `ล้างตัวกรอง` action that clears every control, including Select2/AJAX selects, then reloads data without reinitializing the table.
