...
- [ ] เพิ่ม UI card ใน Stock detail, รองรับการเลือก UOM/as-of และปุ่มส่ง bounded rebuild เฉพาะเมื่อ Accounting/Inventory policy อนุมัติ; ห้ามให้ read path ซ่อมยอดเอง
...
- [x] monitoring dashboard แสดง Queue/Run/Partition status และ failed-run alert; เชื่อมรายละเอียด Run เพื่อใช้ recovery runbook ที่มีอยู่แล้ว
...
- Invalid posted-run monitoring (2026-09-12): เพิ่ม blocker `POSTED_RUN_WITHOUT_DELTA` ใน `wms:cost-health`, `wms:cost-gate-readiness` และหน้า Revaluation Queue สำหรับ Run สถานะ `GL_POSTED/COMPLETED` ที่ไม่มี Revaluation Delta; ตรวจ `new_erp_benchmark` แล้วพบ Run #61 และทำให้ health/gates เป็น `false` จนกว่าจะกักกันหรือแก้หลักฐานให้ครบ โดยไม่ mutate ledger
- Invalid posted-run quarantine (2026-09-12): ตรวจ Run #61 แล้วไม่พบ Delta, Applied Delta หรือ Journal ของ Revaluation จึงยกเลิกผ่าน `CostRevaluationRecoveryService` เป็น `CANCELLED` พร้อม Audit โดยเพิ่ม guard ให้ยกเลิกได้เฉพาะ posted/completed no-op ที่ไม่มี Delta และไม่มี Journal; หลัง quarantine health และ Auto Trigger/Apply/GL readiness กลับเป็น `true`, queue pending/failed = 0
- Controlled E2E sign-off evidence (2026-09-12): สร้าง Clean Run #77 จาก Allocation #2195 โดยทดสอบต้นทุน `10 → 11`; Shadow มี 1 Planned Delta มูลค่า `100.00000000`, Approve/Apply สำเร็จ, GL Journal proof 1 รายการ, `stock_projection_delta = expected_inventory_journal = journal_inventory = 100.00000000`, Debit/Credit balance `0.00000000` และ Reconciliation ผ่านจนสถานะ `COMPLETED`; กักกัน no-op Run #76 เป็น `CANCELLED` พร้อม Audit. Gate readiness และ health ผ่านครบ, queue pending/failed = 0
- AVG RECOST canonical read/replay fix (2026-09-12): เพิ่ม `InventoryCostAllocationService::canonicalAsOf()` ให้เลือก RECOST revision ล่าสุดที่ยัง active ต่อ Movement เพียงรายการเดียว และใช้ร่วมกันใน Stock Card, valuation และ `CostTimelineReader`; effective movement value รวมค่าต้นฉบับกับ RECOST ด้วย `SUM(ABS(value))` จึงไม่บวก correction chain ซ้ำและรายการจ่ายออกหลัง Receipt #21 กลับไปใช้ AVG replay เดียวกับ Engine (เช่น Allocation #1659 ได้ `78.34778624` ก่อนแสดงผลเป็น 78.35). เพิ่ม regression assertion สำหรับ query read path; ไม่แก้ Posted ledger เดิม
- Replay input hardening (2026-09-12): `CostTimelineReader` ไม่ใช้ RECOST ที่มี `metadata.revaluation_run_id` เป็น input event ของ Revaluation รอบใหม่ เพราะเป็น output ของ replay เดิม; Engine จึงคำนวณจาก movement/allocation ต้นฉบับและสร้าง delta ชุดล่าสุดได้โดยไม่ compound correction ซ้ำ ขณะที่ read path ยังเลือก canonical RECOST ล่าสุดเพื่อแสดง effective cost
- Cancelled Run retry fix (2026-09-12): `CostRevaluationApplyService` ไม่ reuse idempotency key ของ Run ที่ `CANCELLED` อีกต่อไป; Trigger ใหม่จะสร้าง Run ใหม่ ส่วน retry ของ Run ที่ active/completed ยัง idempotent ตามเดิม และหน้าเอกสารเลือก Run ล่าสุดเมื่อมีประวัติ Run เดิมใน Batch
- Cancelled Batch retry fix (2026-09-12): เมื่อ Batch เดิมมีแต่ Run `CANCELLED` แล้ว Trigger ใหม่จะ rollover เป็น Batch identity ใหม่ด้วย เพื่อไม่ชน unique `(batch_id, partition_key)` และไม่ผูก Run ใหม่กลับไปยัง Batch เก่า
