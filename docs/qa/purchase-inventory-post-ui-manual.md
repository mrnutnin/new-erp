...
- [x] `inventory_post_url` ถูกส่งจาก backend เฉพาะเมื่อ feature gate เปิด, ผู้ใช้มี `purchasing.purchase-documents.inventory-post`, เอกสารเป็น `APPROVED`, `INVOICE` และ `NONE_VAT`
- [x] หน้า detail ใช้เงื่อนไขเดียวกับ DataTable; VAT/Credit Note ไม่แสดงปุ่ม `Post Inventory`
- [x] route ใช้ `purchasing.purchase-documents.inventory-post` แยกจาก Expense Post และมี permission middleware
...
- [x] user ไม่มี `purchasing.purchase-documents.inventory-post` ไม่เห็นปุ่ม และ route ถูกปฏิเสธ
