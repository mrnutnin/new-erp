...
class PurchaseDocumentPdfController extends Controller
{
    ...
    private function pdf(DocumentPdfRenderer $renderer, string $title, string $number, $date, string $status, ?string $party, ?string $description, array $rows, GlobalSettings $settings): Response
    {
        ...
        $bytes = $renderer->renderView('Wms::pdf.purchase-document', compact('title', 'number', 'date', 'status', 'party', 'description', 'rows', 'company') + ['dateFormat' => (string) $settings->value('date_format')]);
    ...
    {
        abort_unless((int) $model->warehouse_id === (int) $request->attributes->get('selectedWarehouse')->id, 404);
