...
class PurchaseVarianceApproval extends Model
{
    ...
    protected $fillable = [
        'purchase_document_id', 'status', 'revision', 'actor_id', 'acted_at', 'reason',
        'policy_snapshot', 'match_snapshot', 'evidence_hash', 'recovery_hint',
    ];

    protected function casts(): array
    {
        return ['acted_at' => 'datetime', 'policy_snapshot' => 'array', 'match_snapshot' => 'array', 'revision' => 'integer'];
    }
    ...
    public static function evidenceHash(array $match, PurchaseThreeWayMatchPolicy $policy): string
    {
        return hash('sha256', json_encode([
            'blockers' => array_values($match['blockers'] ?? []),
            'lines' => $match['lines'] ?? [],
            'policy' => [
