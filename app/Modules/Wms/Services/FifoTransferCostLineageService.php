...
/**
 * FIFO-only inbound transfer adapter. It never calls the normal FIFO receipt
 * path: every destination layer points to the source layer and every cost
 * allocation points to the source allocation. No Journal/GL rows are made.
 */
final class FifoTransferCostLineageService
{
    ...
    public function inbound(TransferLine $line, Transfer $transfer, string $quantity, string $baseQuantity, string $commandKey, User $actor, int $warehouseId, string $eventType, ?string $businessDate = null): StockMovement
    {
        if ($this->settings->value('inventory_costing_method') !== 'FIFO') {
            throw ValidationException::withMessages(['inventory_costing_method' => 'Transfer FIFO adapter ใช้ได้เมื่อ policy เป็น FIFO เท่านั้น']);
        }
        if (! in_array($eventType, ['ACCEPT', 'REJECT'], true)) {
            throw ValidationException::withMessages(['event_type' => 'Transfer FIFO lineage รองรับเฉพาะ Accept/Reject']);
        }
        if ((int) $line->transfer_id !== (int) $transfer->id) {
            throw ValidationException::withMessages(['transfer_line' => 'Transfer line ไม่ตรงกับเอกสาร']);
        }
        ...
        return DB::transaction(function () use ($line, $transfer, $quantity, $baseQuantity, $commandKey, $actor, $warehouseId, $eventType, $businessDate): StockMovement {
            $source = StockMovement::query()
                ->where('source_type', 'WMS_TRANSFER')->where('source_id', (string) $transfer->id)
                ->where('transfer_key', "transfer:{$transfer->id}:line:{$line->id}")
                ->where('direction', 'OUT')->where('status', 'POSTED')->lockForUpdate()->firstOrFail();
            ...
            if ((int) $targetWarehouse !== $warehouseId) {
                throw ValidationException::withMessages(['warehouse' => 'Warehouse context ไม่ตรงกับปลายทางของ FIFO transfer']);
            }
            $businessDate = $businessDate ?: $transfer->document_date->format('Y-m-d');
            app(BackdatedMovementGate::class)->assertOpen($businessDate);
            ...
            $movement = $this->movements->recordIntent([
                ...
                'business_date' => $businessDate, 'source_type' => 'WMS_TRANSFER', 'source_id' => (string) $transfer->id,
                'source_reference' => $transfer->document_number, 'transfer_key' => "transfer:{$transfer->id}:line:{$line->id}",
                'idempotency_key' => $key.':movement', 'metadata' => ['transfer_id' => $transfer->id, 'transfer_line_id' => $line->id, 'transfer_event' => $eventType, 'fifo_lineage' => true, 'source_movement_id' => $source->id],
                'created_by' => $actor->id,
            ]);
            ...
            if ($movement->status !== 'DRAFT') {
                throw ValidationException::withMessages(['status' => 'FIFO transfer movement อยู่ในสถานะที่ retry ไม่ได้']);
            }
            ...
            $acceptedAllocations = CostAllocation::query()
                ->where('allocation_type', 'TRANSFER')
                ->where('direction', 'IN')
                ->where('status', '!=', 'REVERSED')
                ->whereHas('movement', function (Builder $query) use ($transfer, $line): void {
                    $query->where('source_type', 'WMS_TRANSFER')
                        ->where('source_id', (string) $transfer->id)
                        ->where('metadata->transfer_line_id', $line->id)
                        ->where('metadata->transfer_event', 'ACCEPT');
                })
                ->get(['parent_allocation_id', 'quantity']);
            $acceptedByParent = $acceptedAllocations->groupBy('parent_allocation_id')->map(
                fn ($rows): BigDecimal => $rows->reduce(
                    fn (BigDecimal $sum, CostAllocation $allocation): BigDecimal => $sum->plus(BigDecimal::of((string) $allocation->quantity)),
                    BigDecimal::zero(),
                ),
            );
            $remaining = BigDecimal::of($baseQuantity);
            if ($sourceAllocations->isEmpty()) {
                throw ValidationException::withMessages(['cost_lineage' => 'ไม่พบ FIFO source allocation ที่เป็นต้นทุน final']);
            }
            ...
            foreach ($sourceAllocations as $sourceAllocation) {
                ...
                $alreadyAccepted = $acceptedByParent->get($sourceAllocation->id, BigDecimal::zero());
                $available = BigDecimal::of((string) $sourceAllocation->quantity)->minus($alreadyAccepted);
                if ($available->isLessThanOrEqualTo(0)) {
                    continue;
                }
                $take = $available;
                if ($take->isGreaterThan($remaining)) {
                    $take = $remaining;
                }
                ...
                $parentLayerId = $sourceAllocation->stock_cost_layer_id ?: StockCostLayer::query()->where('warehouse_id', $source->warehouse_id)->where('item_id', $line->item_id)->where('uom_id', $line->uom_id)->where('method', 'FIFO')->orderByDesc('id')->value('id');
                $layer = StockCostLayer::query()->firstOrCreate(['lineage_key' => $lineageKey], [
                    'warehouse_id' => $targetWarehouse, 'item_id' => $line->item_id, 'uom_id' => $line->uom_id,
                    'source_movement_id' => $movement->id, 'parent_layer_id' => $parentLayerId,
                    'original_quantity' => $take, 'remaining_quantity' => $take, 'unit_cost' => $unitCost,
                    'method' => 'FIFO', 'cost_status' => 'FINAL', 'business_date' => $movement->business_date,
                ]);
                $allocation = $this->allocations->record($movement, 'FIFO', [
                    'stock_cost_layer_id' => $layer->id, 'parent_allocation_id' => $sourceAllocation->id,
                    'allocation_type' => 'TRANSFER', 'direction' => 'IN', 'cost_status' => 'FINAL',
                    'quantity' => $take, 'unit_cost' => $unitCost, 'value' => $take->multipliedBy($unitCost),
            ...
            if ($remaining->isPositive()) {
                throw ValidationException::withMessages(['quantity' => 'จำนวน Transfer เกิน FIFO source allocation ที่พร้อมย้าย']);
            }
