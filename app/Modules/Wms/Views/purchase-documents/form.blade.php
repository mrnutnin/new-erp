...
@push('scripts')
    <script>
        $(function () {
            ...
            function togglePurchaseMode() { var inventory=purchaseMode==='INVENTORY', credit=$('[name="document_type"]').val()==='CREDIT_NOTE'; $('#purchase-document-form').toggleClass('purchase-mode-inventory',inventory).toggleClass('purchase-mode-expense',!inventory); $('.purchase-gr-picker-wrap').toggle(inventory); $('.js-expense-only').toggle(!inventory); $('#purchase-gr-picker').prop('disabled', !inventory || credit || !$('#purchase-supplier').val()); $('#add-purchase-line span').text(inventory?'เพิ่มรายการสินค้า':'เพิ่มรายการบริการ'); $('#purchase-mode-hint').text(inventory?'เลือก Goods Receipt เพื่อดึงสินค้า จำนวน และราคาจาก PO':'เลือกได้เฉพาะรายการประเภทบริการ และไม่ต้องอ้างอิง PO/GR'); $lines.find('.purchase-line').each(function(){ mergeInventoryFields($(this)); }); $lines.find('.js-purchase-item').each(function(){ var $select=$(this); if ($select.hasClass('select2-hidden-accessible')) $select.select2('destroy'); initItem($select); }); }
            function toggleType() { var credit=$('[name="document_type"]').val()==='CREDIT_NOTE'; $('.js-original-wrap').toggle(credit); $('#purchase-original').prop('required',credit); $('#purchase-payment-term').prop('required',!credit); togglePurchaseMode(); if(!credit) $('#purchase-original').val(null).trigger('change'); }
            ...
            toggleType(); $('#purchase-tax-treatment').trigger('change'); total(); window.erpAjaxForm({ form:'#purchase-document-form', redirect:true });
        });
    </script>
@endpush
