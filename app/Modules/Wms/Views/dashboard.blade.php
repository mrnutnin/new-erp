...
@section('title', 'WMS Dashboard | MintERP')

@section('content')
<div class="container-fluid px-3 px-lg-4 py-4 module-dashboard module-dashboard--wms" id="wms-dashboard" data-url="{{ route('wms.dashboard.data', ['section' => '__section__']) }}">
    <div class="module-dashboard-hero d-flex flex-wrap justify-content-between align-items-center gap-3 mb-4">
        <div><p class="module-dashboard-kicker mb-2"><span></span>WMS / INVENTORY CONTROL</p><h1>ศูนย์ควบคุมคลังสินค้า</h1><p class="mb-0">ติดตามสต็อก งานค้าง ต้นทุน และความเคลื่อนไหวของคลังที่เลือก</p></div>
    ...
    <section class="row g-3 mb-4 module-dashboard-summary" aria-label="WMS summary">
...
$(function(){
    ...
    $.getJSON(endpoint('trend')).done(function(data){$('#wms-trend-state').remove();if(!window.Chart){return;}new Chart(document.getElementById('wms-trend-chart'),{type:'bar',data:{labels:data.labels,datasets:[{label:'รับเข้า',data:data.in,backgroundColor:'#22a879',borderRadius:6},{label:'จ่ายออก',data:data.out,backgroundColor:'#df5262',borderRadius:6}]},options:{responsive:true,maintainAspectRatio:false,plugins:{tooltip:{callbacks:{label:function(c){return c.dataset.label+': '+Number(c.raw||0).toLocaleString('th-TH');}}}},scales:{y:{beginAtZero:true,ticks:{callback:function(v){return Number(v).toLocaleString('th-TH');}}}}}});}).fail(function(){$('#wms-trend-state').text('โหลดกราฟไม่สำเร็จ');});
