...
$(function () {
    ...
    var dataTable = table.DataTable($.extend(true, {}, window.erpDataTableDefaults, {
        ajax: table.data('url'), order: [[1, 'desc'], [0, 'desc']], buttons: [window.erpExcelButton(table)], columns: [
            {data:'document_number', render:escape}, {data:'document_date', render:escape}, @if($isIncoming){data:null, render:function (value, type, row) { return '<a class="link-primary" href="'+escape.display(row.detail_url)+'">'+escape.display(row.document_number || '-')+'</a><div class="small text-secondary">'+escape.display(row.source_label || '-')+'</div>'; }}, @endif {data:'source_label', render:escape}, {data:'destination_label', render:escape},
            ...
            {data:null, render:function (value, type, row) { return '<div class="fw-semibold">'+escape.display(row.destination_receipt_status || '-')+'</div><div class="small text-secondary">'+escape.display(row.destination_receipt_summary || '-')+'</div>'; }},
