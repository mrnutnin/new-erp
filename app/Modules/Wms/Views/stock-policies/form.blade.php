    ...
    <form id="stock-policy-form" method="POST" action="{{$policy->exists?route('wms.stock-policies.update',$policy):route('wms.stock-policies.store')}}">
        ...
        <div class="card border-0 shadow-sm">
            <div class="card-body p-4 row g-3">
                ...
                <div class="col-md-4"><label class="form-label">ปริมาณขั้นต่ำ (Min)</label><input class="form-control" name="min_quantity" type="number" min="0" step="{{$quantityStep}}" value="{{ old('min_quantity', \App\Modules\Wms\Support\WmsDecimal::input($policy->min_quantity ?? 0)) }}" required><div class="invalid-feedback" data-error-for="min_quantity"></div></div>
                <div class="col-md-4"><label class="form-label">ปริมาณสูงสุด (Max)</label><input class="form-control" name="max_quantity" type="number" min="0" step="{{$quantityStep}}" value="{{ old('max_quantity', \App\Modules\Wms\Support\WmsDecimal::input($policy->max_quantity ?? 0)) }}" required><div class="invalid-feedback" data-error-for="max_quantity"></div></div>
                <div class="col-md-4"><label class="form-label">จำนวนที่เติมเมื่อถึง Min</label><input class="form-control" name="reorder_quantity" type="number" min="0" step="{{$quantityStep}}" value="{{ old('reorder_quantity', \App\Modules\Wms\Support\WmsDecimal::input($policy->reorder_quantity ?? 0)) }}" required><div class="invalid-feedback" data-error-for="reorder_quantity"></div></div>
