...
<div class="container-fluid px-3 px-lg-4 py-4">
    <div class="d-flex flex-column flex-lg-row justify-content-between align-items-lg-end gap-3 mb-4">
        ...
        <div class="d-flex flex-wrap gap-2">
            <a class="btn btn-outline-secondary" href="{{ $productionMode ? route('wms.production.material-issues.index') : route('wms.issues.index') }}"><i class="bx bx-arrow-back me-1" aria-hidden="true"></i>กลับ</a>
            @if($document->status === 'DRAFT' && auth()->user()->hasPermission('wms.issues.delete'))
                <button class="btn btn-outline-danger js-delete" type="button" data-url="{{ $productionMode ? route('wms.production.material-issues.destroy', $document) : route('wms.issues.destroy', $document) }}"><i class="bx bx-trash me-1" aria-hidden="true"></i>ลบ Draft</button>
            @endif
            ...
            @if(in_array($document->status, ['DRAFT', 'APPROVED'], true) && auth()->user()->hasPermission('wms.issues.approve'))
                <button class="btn btn-outline-danger js-cancel" type="button" data-url="{{ $productionMode ? route('wms.production.material-issues.cancel', $document) : route('wms.issues.cancel', $document) }}"><i class="bx bx-x-circle me-1" aria-hidden="true"></i>ยกเลิก</button>
            @endif
            @if($document->status === 'POSTED')
                @if($productionMode && auth()->user()->hasPermission('wms.inventory-adjustments.create'))
                    <a class="btn btn-dark" href="{{ route('wms.production.finished-receipts.create', ['source_issue_id' => $document->id]) }}"><i class="bx bx-package me-1" aria-hidden="true"></i>สร้างใบรับผลิตเสร็จ</a>
                @endif
