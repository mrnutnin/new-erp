...
$(function(){
 ...
 const table=tableElement.DataTable($.extend(true,{},window.erpDataTableDefaults,{processing:true,serverSide:true,order:[[2,'desc']],ajax:{url:'{{ route('wms.inventory-adjustments.data') }}',data:function(data){data.status=filters.find('.js-wms-filter-status').val();data.date_from=filters.find('.js-wms-filter-from').val();data.date_to=filters.find('.js-wms-filter-to').val();}},buttons:[window.erpExcelButton(tableElement)],columns:[
