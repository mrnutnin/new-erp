...
@php($statusClasses = ['DRAFT' => 'app-status-neutral', 'REGISTERED' => 'app-status-info', 'ACTIVE' => 'app-status-success', 'SUSPENDED' => 'app-status-warning', 'UNDER_REPAIR' => 'app-status-warning', 'HELD_FOR_DISPOSAL' => 'app-status-warning', 'DISPOSED' => 'app-status-danger', 'WRITTEN_OFF' => 'app-status-danger'])
...
<div class="container-fluid px-3 px-lg-4 py-4">
    <div class="d-flex flex-wrap justify-content-between align-items-start gap-3 mb-4">
        ...
        <div class="d-flex align-items-center gap-2">
            <span class="badge {{ $statusClasses[$asset->status] ?? 'app-status-neutral' }}">{{ $statusLabels[$asset->status] ?? $asset->status }}</span>
            <a class="btn btn-app-soft" href="{{ route('asset.assets.label', $asset) }}" target="_blank"><i class="bx bx-barcode me-1" aria-hidden="true"></i>พิมพ์ป้าย</a>
            @if ($asset->status === 'DRAFT' && auth()->user()->hasPermission('asset.register.update'))
                <a class="btn btn-app-soft" href="{{ route('asset.assets.edit', $asset) }}"><i class="bx bx-edit me-1" aria-hidden="true"></i>แก้ไข</a>
            @endif
            @if ($asset->status === 'DRAFT' && auth()->user()->hasPermission('asset.capitalizations.create'))
                <a class="btn btn-app-primary" href="{{ route('asset.capitalizations.create', ['source_type' => 'MANUAL_RECLASS', 'asset_id' => $asset->id]) }}"><i class="bx bx-book-add me-1" aria-hidden="true"></i>ตั้งทุนและลงบัญชี</a>
            @endif
            @if ($asset->status === 'ACTIVE' && auth()->user()->hasPermission('asset.capitalizations.create'))
                <a class="btn btn-app-primary" href="{{ route('asset.additions.create', ['source_type' => 'MANUAL_RECLASS', 'asset_id' => $asset->id]) }}"><i class="bx bx-plus-circle me-1" aria-hidden="true"></i>เพิ่มมูลค่า</a>
            @endif
            <a class="btn btn-app-soft" href="{{ route('asset.assets.index') }}"><i class="bx bx-arrow-back me-1" aria-hidden="true"></i>กลับหน้ารายการ</a>
            @if ($asset->status === 'DRAFT' && auth()->user()->hasPermission('asset.register.update'))
        ...
        <div class="tab-pane fade" id="asset-depreciation">
            ...
            @forelse ($asset->depreciationBooks as $book)
                ...
                <div class="card border-0 shadow-sm mb-3"><div class="card-body p-4">
                    <div class="d-flex flex-wrap justify-content-between align-items-start gap-3 mb-3"><div><div class="d-flex align-items-center gap-2 mb-1"><h2 class="h6 mb-0">{{ $book->book_type === 'BOOK' ? 'ค่าเสื่อมทางบัญชี' : 'ค่าเสื่อมทางภาษี' }}</h2><span class="badge text-bg-light">เส้นตรง</span></div><p class="small text-secondary mb-0">เริ่ม {{ $book->start_date?->format($dateFormat) ?? '-' }} · อายุ {{ number_format($book->useful_life_months) }} เดือน · คิดล่าสุด {{ $book->last_depreciation_date?->format($dateFormat) ?? '-' }}</p></div><div class="btn-group btn-group-sm" role="group" aria-label="วิธีเฉลี่ยค่าเสื่อม"><button class="btn btn-app-soft js-depreciation-method" data-book="{{ $book->id }}" data-method="FULL_MONTH" type="button">เต็มเดือน</button><button class="btn btn-app-soft active js-depreciation-method" data-book="{{ $book->id }}" data-method="DAILY" type="button">รายวัน</button></div></div>
                    @if ($preview)
...
<script>
$(function () {
    window.erpAjaxDelete({button:'.js-delete-asset',redirect:@json(route('asset.assets.index')),confirm:'ยืนยันการลบทะเบียนสินทรัพย์ร่างนี้หรือไม่?'});
    ...
    function loadAttachments(){ $.getJSON(listUrl).done(function(response){ var rows=response.data||[],photos=rows.filter(function(row){return row.preview_url;});$gallery.toggleClass('d-none',!photos.length).html(photos.map(function(row){return '<div class="col-6 col-md-3"><button class="btn p-0 border w-100 js-preview-asset-photo" data-url="'+text.display(row.preview_url)+'" data-name="'+text.display(row.original_name)+'" type="button"><img class="img-fluid rounded" src="'+text.display(row.preview_url)+'" alt="'+text.display(row.original_name)+'"></button></div>';}).join(''));$rows.html(rows.length?rows.map(function(row){var actions=(row.preview_url?'<button class="btn btn-sm btn-app-soft js-preview-asset-photo" data-url="'+text.display(row.preview_url)+'" data-name="'+text.display(row.original_name)+'" type="button">ดูรูป</button> ':'')+'<a class="btn btn-sm btn-app-soft" href="'+text.display(row.download_url)+'">ดาวน์โหลด</a>';if(row.delete_url){actions+=' <button class="btn btn-sm btn-app-danger js-delete-attachment" data-url="'+text.display(row.delete_url)+'" type="button">ลบ</button>';}return '<tr><td>'+text.display(types[row.file_type]||row.file_type)+'</td><td>'+text.display(row.original_name)+'</td><td>'+Number(row.bytes).toLocaleString()+' bytes</td><td>'+text.display(row.uploaded_by)+'</td><td>'+text.display(row.uploaded_at||'-')+'</td><td class="text-end">'+actions+'</td></tr>';}).join(''):'<tr><td colspan="6" class="text-center text-secondary py-4">ยังไม่มีเอกสารแนบ</td></tr>'); }).fail(function(){$rows.html('<tr><td colspan="6" class="text-center text-danger py-4">โหลดเอกสารแนบไม่สำเร็จ</td></tr>');}); }
    loadAttachments();
