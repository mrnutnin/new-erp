...
@section('content')
    <div class="container-fluid px-3 px-lg-4 py-4">
        <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-end gap-3 mb-4"><div><p class="eyebrow mb-2">ASSET / {{ $isAddition ? 'ADDITION' : 'CAPITALIZATION' }}</p><h1 class="h3 mb-2">{{ $documentLabel }}</h1><p class="text-secondary mb-0">{{ $isAddition ? 'เพิ่มต้นทุนของสินทรัพย์ที่ใช้งานอยู่ โดยไม่เปลี่ยนต้นทุนตั้งต้น' : 'รับรู้ต้นทุนจากใบซื้อหรือยอดยกมาเข้าสู่สินทรัพย์' }}</p></div>@if (auth()->user()->hasPermission('asset.capitalizations.create'))<a class="btn btn-dark" href="{{ route($routePrefix.'.create') }}"><i class="bx bx-plus me-1" aria-hidden="true"></i>สร้าง{{ $isAddition ? 'ใบเพิ่มมูลค่า' : 'ใบรับรู้' }}</a>@endif</div>
...
$(function () {
    ...
    window.erpAjaxDelete({button:'.js-delete-capitalization',reload:'#capitalizations-table',confirm:@json('ยืนยันการลบ'.$documentLabel.'ร่างนี้หรือไม่?')});
});
</script>
@endpush
