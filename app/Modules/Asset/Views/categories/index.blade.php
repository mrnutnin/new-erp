...
@section('title', 'หมวดสินทรัพย์ | MintERP')
...
@section('content')
    <div class="container-fluid px-3 px-lg-4 py-4">
        <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-end gap-3 mb-4">
            ...
            @if (auth()->user()->hasPermission('asset.categories.manage'))<a class="btn btn-app-primary" href="{{ route('asset.categories.create') }}"><i class="bx bx-plus me-1" aria-hidden="true"></i>เพิ่มหมวดสินทรัพย์</a>@endif
        </div>
        <div class="card border-0 shadow-sm mb-4"><div class="card-body p-3 p-lg-4"><div class="d-flex flex-wrap justify-content-between align-items-center gap-2 mb-3"><div><h2 class="h5 mb-1">ตัวกรอง</h2><p class="small text-secondary mb-0">กรองตามสถานะก่อนดูรายการ</p></div><button class="btn btn-sm btn-app-soft" id="category-filter-reset" type="button"><i class="bx bx-reset me-1" aria-hidden="true"></i>ล้างตัวกรอง</button></div><div class="row g-3"><div class="col-12 col-md-4"><label class="form-label" for="category-filter-status">สถานะ</label><select class="form-select" id="category-filter-status"><option value="">ทุกสถานะ</option><option value="1">ใช้งาน</option><option value="0">ปิดใช้งาน</option></select></div></div></div></div>
...
$(function () {
    var $table = $('#categories-table'), text = $.fn.dataTable.render.text(), columns = [
        ...
        {data:'is_depreciable', name:'is_depreciable', searchable:false, render:function(v,t){return t === 'display' ? '<span class="badge '+(v?'app-status-success':'app-status-neutral')+'">'+(v?'คิดค่าเสื่อม':'ไม่คิดค่าเสื่อม')+'</span>' : v;}},
        ...
        {data:'is_active', name:'is_active', searchable:false, render:function(v,t){return t === 'display' ? '<span class="badge '+(v?'app-status-success':'app-status-neutral')+'">'+(v?'ใช้งาน':'ปิดใช้งาน')+'</span>' : v;}}
    ];
    if ($table.data('can-manage')) columns.push({data:null, orderable:false, searchable:false, className:'text-end', render:function(v,t,row){if(t !== 'display') return ''; var actions=[]; if(row.edit_url) actions.push('<a class="btn btn-sm btn-app-soft" href="'+text.display(row.edit_url)+'" title="แก้ไข" aria-label="แก้ไข"><i class="bx bx-edit" aria-hidden="true"></i></a>'); if(row.delete_url) actions.push('<button class="btn btn-sm btn-app-danger js-delete-category" data-url="'+text.display(row.delete_url)+'" type="button" title="ลบ" aria-label="ลบ"><i class="bx bx-trash" aria-hidden="true"></i></button>'); return actions.join(' ');}});
