...
$(function () {
    ...
    var table=$table.DataTable($.extend(true,{},window.erpDataTableDefaults,{ajax:{url:$table.data('url'),data:function(data){data.status=$('#maintenance-status').val();data.priority=$('#maintenance-priority').val();data.date_from=$('#maintenance-date-from').val();data.date_to=$('#maintenance-date-to').val();}},order:[[1,'desc']],buttons:[window.erpExcelButton($table)],columns:[
        ...
        {data:'status',render:function(value,type){return type==='display'?'<span class="badge '+(badges[value]||'app-status-neutral')+'">'+text.display(labels[value]||value)+'</span>':value;}},
        {data:'show_url',orderable:false,searchable:false,className:'text-end',render:function(value,type){return type==='display'?'<a class="btn btn-sm btn-app-soft" href="'+text.display(value)+'" title="ดูรายละเอียด" aria-label="ดูรายละเอียด"><i class="bx bx-file-find" aria-hidden="true"></i></a>':value;}}
    ]}));
