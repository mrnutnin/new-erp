...
class AssetCapitalizationController extends Controller
{
    ...
    {
        return view('Asset::capitalizations.index');
    }
    ...
    public function data(Request $request): JsonResponse
    {
        return DataTables::eloquent(AssetCapitalization::query()->withCount('lines')
            ...
            ->addColumn('show_url', fn (AssetCapitalization $row) => route('asset.capitalizations.show', $row))
            ->addColumn('delete_url', fn (AssetCapitalization $row) => $row->status === 'DRAFT' && $request->user()->hasPermission('asset.capitalizations.create') ? route('asset.capitalizations.destroy', $row) : null)
            ->toJson();
    }
    ...
    public function create(Request $request): View
    {
        ...
        if ($request->string('source_type')->toString() === 'MANUAL_RECLASS' && $request->integer('asset_id')) {
            $manualAsset = Asset::query()->with('category:id,code,name')->where('branch_id', $this->branchId($request))
                ->where('status', 'DRAFT')->findOrFail($request->integer('asset_id'));
        }

        return view('Asset::capitalizations.form', ['manualAsset' => $manualAsset]);
    }
    ...
    public function store(
        ...
        $capitalization = DB::transaction(function () use ($request, $service, $sequences): AssetCapitalization {
            ...
            $sequence = DocumentSequence::query()->whereNull('warehouse_id')->where('document_type', 'ASSET_CAPITALIZATION')->where('is_active', true)->lockForUpdate()->first();
            if (! $sequence) {
                throw ValidationException::withMessages(['document_date' => 'ยังไม่ได้ตั้งค่าเลขใบรับรู้สินทรัพย์']);
            }
            ...
            $capitalization = $service->createDraft($branch, [...$request->validated(), 'document_number' => $number], $request->user());
            $sequences->recordIssued($sequence, $number, 'asset_capitalizations', $capitalization->id, $date, $request->user()->id);

            return $capitalization;
        });

        return response()->json(['status' => true, 'msg' => 'สร้างใบรับรู้สินทรัพย์ร่างแล้ว', 'redirect' => route('asset.capitalizations.show', $capitalization)]);
    }

    public function show(Request $request, AssetCapitalization $capitalization): View
    {
        return view('Asset::capitalizations.show', ['capitalization' => $this->scoped($request, $capitalization)->load([
    ...
    public function destroy(Request $request, AssetCapitalization $capitalization, AuditLogger $audit): JsonResponse
    {
        ...
        return response()->json(['status' => true, 'msg' => 'ลบใบรับรู้สินทรัพย์ร่างแล้ว', 'redirect' => route('asset.capitalizations.index')]);
    }
    ...
    public function submit(Request $request, AssetCapitalization $capitalization, AssetCapitalizationService $service): JsonResponse
    {
        ...
        return $this->changed('ส่งใบรับรู้สินทรัพย์เพื่ออนุมัติแล้ว', $capitalization);
    }
    ...
    public function approve(Request $request, AssetCapitalization $capitalization, AssetCapitalizationService $service): JsonResponse
    {
        ...
        return $this->changed('อนุมัติใบรับรู้สินทรัพย์แล้ว', $capitalization);
    }
    ...
    public function post(Request $request, AssetCapitalization $capitalization, AssetCapitalizationService $service): JsonResponse
    {
        ...
        return $this->changed('ลงบัญชีใบรับรู้สินทรัพย์แล้ว', $capitalization);
    }
    ...
    public function void(VoidAssetCapitalizationRequest $request, AssetCapitalization $capitalization, AssetCapitalizationService $service, AuditLogger $audit): JsonResponse
    {
        ...
        return $this->changed('ยกเลิกใบรับรู้สินทรัพย์แล้ว', $capitalization);
    }
    ...
    public function reverse(ReverseAssetCapitalizationRequest $request, AssetCapitalization $capitalization, AssetCapitalizationService $service): JsonResponse
    {
        ...
        return $this->changed('กลับรายการใบรับรู้สินทรัพย์แล้ว', $capitalization);
    }
    ...
    public function assetOptions(Request $request): JsonResponse
    {
        ...
        $rows = Asset::query()->with('category:id,code,name')->where('branch_id', $this->branchId($request))->where('status', 'DRAFT')->when($categoryId, fn (Builder $query) => $query->where('asset_category_id', $categoryId))
    ...
    private function changed(string $message, AssetCapitalization $capitalization): JsonResponse
    {
        return response()->json(['status' => true, 'msg' => $message, 'redirect' => route('asset.capitalizations.show', $capitalization)]);
    }

    private function scoped(Request $request, AssetCapitalization $capitalization): AssetCapitalization
    {
        return AssetCapitalization::query()->where('branch_id', $this->branchId($request))->findOrFail($capitalization->id);
    }
