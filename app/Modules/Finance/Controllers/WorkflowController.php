...
class WorkflowController extends Controller
{
    public function index(Request $request, ModuleCapability $capability, WorkflowRuntimeResolver $runtime): View
    {
        $workflows = array_map(function (array $workflow): array {
            ...
            return $workflow;
        }, $runtime->decorate('finance', WorkflowCatalog::for('finance', $capability), auth()->user(), (int) $request->attributes->get('selectedWarehouse')->id));

        return view('Finance::workflow.index', ['program' => $request->attributes->get('selectedProgram'), 'warehouse' => $request->attributes->get('selectedWarehouse'), 'workflows' => $workflows]);
    }
}
