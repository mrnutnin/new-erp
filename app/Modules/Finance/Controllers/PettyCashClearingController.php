...
final class PettyCashClearingController extends Controller
{
    ...
    {
        $query = PettyCashClearing::query()->with('fund.cashBankAccount')->where('warehouse_id', $this->warehouse($request)->id)
    ...
    public function destroy(Request $request, PettyCashClearing $clearing, PettyCashClearingService $service): JsonResponse { $this->scope($request, $clearing); $service->deleteDraft($clearing, $this->warehouse($request), $request->user(), $request); return response()->json(['status' => true, 'msg' => 'ลบร่างเอกสารแล้ว', 'redirect' => route('finance.petty-cash-clearings.index')]); }
