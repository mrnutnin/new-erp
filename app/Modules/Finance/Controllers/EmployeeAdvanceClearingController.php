...
final class EmployeeAdvanceClearingController extends Controller
{
    ...
    {
        $query = EmployeeAdvanceClearing::query()->with('advance.employee')->where('warehouse_id', $this->warehouse($request)->id)
    ...
    public function destroy(Request $request, EmployeeAdvanceClearing $clearing, EmployeeAdvanceClearingService $service): JsonResponse { $this->scope($request, $clearing); $service->deleteDraft($clearing, $this->warehouse($request), $request->user(), $request); return response()->json(['status' => true, 'msg' => 'ลบร่างเอกสารแล้ว', 'redirect' => route('finance.employee-advance-clearings.index')]); }
