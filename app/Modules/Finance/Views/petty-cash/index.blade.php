    ...
    <div class="container-fluid px-3 px-lg-4 py-4">
        <div class="d-flex flex-column flex-lg-row justify-content-between align-items-lg-end gap-3 mb-4">
            ...
            <div class="d-flex flex-wrap gap-2">
                @if (auth()->user()->hasPermission('finance.petty-cash.create'))
                    <a class="btn btn-app-primary" href="{{ route('finance.petty-cash.create') }}"><i class="bx bx-plus me-1" aria-hidden="true"></i>สร้างใบสำคัญ</a>
                @endif
            </div>
        </div>
        ...
        <div class="card border-0 shadow-sm mb-4" aria-labelledby="petty-cash-filter-title">
            <div class="card-body p-3 p-lg-4">
                <div class="d-flex justify-content-between align-items-center gap-3 mb-3"><div><h2 id="petty-cash-filter-title" class="h5 mb-1">ตัวกรอง</h2><p class="small text-secondary mb-0">กรองข้อมูลก่อนดูรายการ</p></div><button type="button" id="petty-cash-filter-reset" class="btn btn-sm btn-app-soft">ล้างตัวกรอง</button></div>
                <div class="row g-3">
                    ...
                    <div class="col-md-4"><label class="form-label" for="petty-cash-date-from">วันที่เอกสารตั้งแต่</label><input id="petty-cash-date-from" class="form-control" type="date"></div>
                    <div class="col-md-4"><label class="form-label" for="petty-cash-date-to">วันที่เอกสารถึง</label><input id="petty-cash-date-to" class="form-control" type="date"></div>
                </div>
            </div>
        </div>
...
@push('scripts')
<script>
$(function(){var $table=$('#petty-cash-table'),text=$.fn.dataTable.render.text(),filters=function(){return {status:$('#petty-cash-status').val(),petty_cash_fund_id:$('#petty-cash-fund').val(),date_from:$('#petty-cash-date-from').val(),date_to:$('#petty-cash-date-to').val()};},labels={DRAFT:'ร่าง',SUBMITTED:'รออนุมัติ',APPROVED:'อนุมัติแล้ว',POSTED:'ลงบัญชีแล้ว',REVERSED:'ยกเลิกรายการแล้ว',VOID:'ยกเลิก'},classes={DRAFT:'app-status-neutral',SUBMITTED:'app-status-info',APPROVED:'app-status-info',POSTED:'app-status-success',REVERSED:'app-status-danger',VOID:'app-status-danger'};var table=$table.DataTable($.extend(true,{},window.erpDataTableDefaults,{ajax:{url:$table.data('url'),data:function(d){$.extend(d,filters());}},order:[[1,'desc']],buttons:[window.erpExcelButton($table,filters)],columns:[{data:'document_number',name:'finance_petty_cash_vouchers.document_number',render:text.display},{data:'document_date_label',name:'finance_petty_cash_vouchers.document_date',render:text.display},{data:'fund_label',name:'finance_petty_cash_funds.id',render:text.display},{data:'payee_name',name:'finance_petty_cash_vouchers.payee_name',defaultContent:'—',render:text.display},{data:'total_amount',name:'finance_petty_cash_vouchers.total_amount',className:'text-end',render:$.fn.dataTable.render.number(',','.',2)},{data:'status',name:'finance_petty_cash_vouchers.status',render:function(v,t){return t==='display'?'<span class="badge '+(classes[v]||'app-status-neutral')+'">'+text.display(labels[v]||v)+'</span>':v;}},{data:null,orderable:false,searchable:false,className:'text-end text-nowrap',render:function(v,t,r){if(t!=='display')return '';var a=['<a class="btn btn-sm btn-app-soft" href="'+text.display(r.show_url)+'" title="ดูรายละเอียด" aria-label="ดูรายละเอียด"><i class="bx bx-file-find" aria-hidden="true"></i></a>'];if(r.edit_url)a.push('<a class="btn btn-sm btn-app-soft" href="'+text.display(r.edit_url)+'" title="แก้ไข" aria-label="แก้ไข"><i class="bx bx-edit" aria-hidden="true"></i></a>');if(r.delete_url)a.push('<button class="btn btn-sm btn-app-danger js-delete-petty-cash" type="button" data-url="'+text.display(r.delete_url)+'" title="ลบร่าง" aria-label="ลบร่าง"><i class="bx bx-trash" aria-hidden="true"></i></button>');return a.join(' ');}}]}));$('#petty-cash-status,#petty-cash-fund,#petty-cash-date-from,#petty-cash-date-to').on('change',function(){table.ajax.reload();});$('#petty-cash-filter-reset').on('click',function(){$('#petty-cash-status,#petty-cash-fund,#petty-cash-date-from,#petty-cash-date-to').val('');table.ajax.reload();});window.erpAjaxDelete({button:'.js-delete-petty-cash',reload:'#petty-cash-table',confirm:'ยืนยันการลบใบสำคัญเงินสดย่อยฉบับร่างนี้หรือไม่?',confirmButtonText:'ลบร่าง',cancelButtonText:'กลับ'});});
</script>
@endpush
