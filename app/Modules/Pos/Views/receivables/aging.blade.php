...
<div class="container-fluid px-3 px-lg-4 py-4">
    ...
    <div class="card border-0 shadow-sm mb-3"><div class="card-body p-3 p-lg-4"><div class="d-flex flex-wrap justify-content-between align-items-center gap-2 mb-3"><div><h2 class="h5 mb-1">ตัวกรองอายุหนี้</h2><p class="text-secondary small mb-0">เลือกวันที่อ้างอิงและลูกค้าก่อนค้นหาจากตาราง</p></div><button class="btn btn-sm btn-app-soft" id="aging-reset" type="button"><i class="bx bx-reset me-1" aria-hidden="true"></i>ล้างตัวกรอง</button></div><div class="row g-3 align-items-end">
...
$(function () {
    ...
    const dt = table.DataTable($.extend(true, {}, window.erpDataTableDefaults, {ajax: {url: table.data('url'), data: d => { d.as_of = $('#aging-as-of').val(); d.party_id = $('#aging-party').val(); }}, order: [[7, 'desc']], buttons: [window.erpExcelButton(table)], columns: [window.erpRowNumberColumn(),
        ...
        {data: 'details_url', orderable: false, searchable: false, className: 'text-end text-nowrap', render: (url, type) => type === 'display' ? '<a class="btn btn-sm btn-app-soft" href="' + esc.display(url) + '" title="ดูรายละเอียดลูกหนี้" aria-label="ดูรายละเอียดลูกหนี้"><i class="bx bx-list-ul" aria-hidden="true"></i></a>' : ''}
    ]}));
    $('#aging-filter').on('click', () => dt.ajax.reload());
    $('#aging-reset').on('click', () => { $('#aging-as-of').val(@json(today()->toDateString())); $('#aging-party').val(null).trigger('change'); dt.ajax.reload(); });
