...
<div class="container-fluid px-3 px-lg-4 py-4">
    ...
    <div class="card border-0 shadow-sm mb-3"><div class="card-body p-3 p-lg-4"><div class="d-flex flex-wrap justify-content-between align-items-center gap-2 mb-3"><div><h2 class="h5 mb-1">ตัวกรองลูกหนี้คงค้าง</h2><p class="text-secondary mb-0 small">กรองก่อนค้นหาจากตาราง</p></div><button id="receivable-reset" class="btn btn-sm btn-app-soft" type="button"><i class="bx bx-reset me-1"></i>ล้างตัวกรอง</button></div><div class="row g-3 align-items-end">
        ...
        <div class="col-md-1"><button class="btn btn-app-primary w-100" id="receivable-filter" type="button"><i class="bx bx-filter-alt" aria-hidden="true"></i><span>ใช้ตัวกรอง</span></button></div>
    </div></div></div>
    <div class="card border-0 shadow-sm"><div class="card-body p-3 p-lg-4"><h2 class="h5 mb-1">รายการใบขายเชื่อ</h2><p class="text-secondary mb-3 small">เปิด Statement เพื่อตรวจสอบยอดและรับชำระหนี้</p><div class="table-responsive"><table id="receivables-table" class="table table-hover align-middle w-100 mb-0" data-url="{{ route('pos.receivables.data') }}"><thead class="table-light"><tr><th>ลำดับ</th><th>เลขที่ IV</th><th>วันที่เอกสาร</th><th>ครบกำหนด</th><th>ลูกค้า</th><th class="text-end">ยอดตั้งหนี้</th><th class="text-end">คงเหลือ</th><th>สถานะ</th><th class="text-end">จัดการ</th></tr></thead></table></div></div></div>
</div>
@endsection
...
$(function () {
    ...
    const dt = table.DataTable($.extend(true, {}, window.erpDataTableDefaults, {ajax: {url: table.data('url'), data: d => { d.due_from = $('#receivable-due-from').val(); d.due_to = $('#receivable-due-to').val(); d.party_id = $('#receivable-party').val(); d.status = $('#receivable-status').val(); }}, order: [[3, 'asc']], buttons: [window.erpExcelButton(table)], columns: [window.erpRowNumberColumn(),
        ...
        {data: null, orderable: false, searchable: false, className: 'text-end', render: (_, __, r) => r.show_url ? '<a class="btn btn-sm btn-app-soft" title="ดู Statement" aria-label="ดู Statement" href="' + esc.display(r.show_url) + '"><i class="bx bx-file-find"></i></a>' : '—'}
    ]}));
    $('#receivable-filter').on('click', () => dt.ajax.reload());
    $('#receivable-reset').on('click', () => { $('#receivable-due-from,#receivable-due-to').val(''); $('#receivable-status').val(''); $('#receivable-party').val(null).trigger('change'); dt.ajax.reload(); });
