    ...
    <div class="d-flex flex-wrap justify-content-between align-items-start gap-3 mb-4">
        ...
        <div class="d-flex flex-wrap justify-content-end gap-2">
            ...
            @if ($x->requires_rfq)
                ...
                @elseif ($x->rfq?->status === 'APPROVED')
                    @if (auth()->user()->hasPermission('pos.sales-quotations.create'))
                        <form class="js-convert d-inline-flex" method="post" action="{{ route('pos.sales-quotations.from-rfq', $x->rfq) }}">@csrf<button class="btn btn-app-primary d-inline-flex align-items-center gap-1" type="submit"><i class="bx bx-plus-circle"></i>สร้างใบเสนอราคา</button></form>
                    @endif
                    @if (auth()->user()->hasPermission('pos.sales-orders.create'))
                        <form class="js-convert d-inline-flex" method="post" action="{{ route('pos.sales-orders.from-rfq', $x->rfq) }}">@csrf<button class="btn btn-app-soft d-inline-flex align-items-center gap-1" type="submit"><i class="bx bx-cart-add"></i>สร้างใบสั่งขาย</button></form>
                    @endif
                @endif
            @elseif ($x->order)
            ...
            @elseif (in_array($x->status, ['DRAFT', 'COMPLETED'], true) && (! $x->quotation || $x->quotation->status === 'CANCELLED'))
                @if (auth()->user()->hasPermission('pos.sales-quotations.create'))
                    <form class="js-convert d-inline-flex" method="post" action="{{ route('pos.sales-quotations.from-intake', $x) }}">@csrf<button class="btn btn-app-primary d-inline-flex align-items-center gap-1" type="submit"><i class="bx bx-plus-circle"></i>สร้างใบเสนอราคา</button></form>
                @endif
                @if (! $x->quotation && auth()->user()->hasPermission('pos.sales-orders.create'))
                    <form class="js-convert d-inline-flex" method="post" action="{{ route('pos.sales-orders.from-intake', $x) }}">@csrf<button class="btn btn-app-soft d-inline-flex align-items-center gap-1" type="submit"><i class="bx bx-cart-add"></i>สร้างใบสั่งขาย</button></form>
                @endif
            @endif
            ...
            @if($canRevise && auth()->user()->hasPermission('pos.sales-intakes.cancel'))<button type="button" class="btn btn-app-danger d-inline-flex align-items-center gap-1 js-intake-cancel" data-url="{{ route('pos.sales-intakes.cancel', $x) }}"><i class="bx bx-x-circle"></i>ยกเลิกเอกสาร</button>@endif
            @include('Pos::partials.draft-delete-button', ['document' => $x, 'permission' => 'pos.sales-intakes.delete', 'url' => route('pos.sales-intakes.destroy', $x)])
        </div>
    </div>
    @if($x->status === 'DRAFT' && $x->requires_rfq)<div class="alert alert-info border-0 shadow-sm mb-4"><strong>ขั้นถัดไป:</strong> สร้าง RFQ เพื่อขออนุมัติราคาก่อนดำเนินการขาย</div>@elseif($x->status === 'DRAFT' && ! $x->requires_rfq)<div class="alert alert-info border-0 shadow-sm mb-4"><strong>ขั้นถัดไป:</strong> สร้างใบเสนอราคาหรือใบสั่งขายตามกระบวนการขายที่ตกลงกับลูกค้า</div>@elseif($x->rfq?->status === 'WAIT')<div class="alert alert-info border-0 shadow-sm mb-4"><strong>ขั้นถัดไป:</strong> รอพิจารณา RFQ ก่อนสร้างเอกสารขาย</div>@elseif($x->quotation || $x->order)<div class="alert alert-success border-0 shadow-sm mb-4"><strong>เอกสารถูกส่งต่อแล้ว</strong> เปิดเอกสารที่เชื่อมโยงเพื่อดำเนินการตามขั้นตอนถัดไป</div>@endif
