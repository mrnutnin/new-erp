...
<h2>ใบสั่งขาย</h2>
<table class="doc-meta"><tbody><tr><td width="34%">เลขที่<br><b>{{ $order->document_number }}</b></td><td width="33%">วันที่เอกสาร<br><b>{{ optional($order->document_date)->format($dateFormat) }}</b></td><td>ใช้ได้ถึง<br><b>{{ optional($order->valid_until)->format($dateFormat) }}</b></td></tr></tbody></table>
@include('Pos::pdf.partials.document-detail-header', ['document' => $order])
...
@foreach($order->lines as $line)<tr><td>{{ $line->line_number }}</td><td>{{ data_get($line->item_snapshot,'code') }} · {{ data_get($line->item_snapshot,'name') }}<br>{{ $line->description }}</td><td>{{ data_get($line->uom_snapshot,'code') }}</td><td class="right">{{ number_format((float) $line->quantity, $decimalPlaces) }}</td><td class="right">{{ number_format((float) $line->unit_price, 2) }}</td><td class="right">{{ number_format((float) $line->discount_amount, 2) }}</td><td class="right">{{ number_format((float) $line->line_total, 2) }}</td></tr>@endforeach
<tr class="total"><td colspan="6" class="right">รวมสุทธิ</td><td class="right">{{ number_format((float) $order->total_amount, 2) }}</td></tr></tbody></table>
@if($order->description)<p>หมายเหตุ: {{ $order->description }}</p>@endif
