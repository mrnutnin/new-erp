...
<div class="container-fluid px-3 px-lg-4 py-4">
    ...
    <div class="d-flex flex-wrap justify-content-between align-items-start gap-3 mb-4"><div><h1 class="h2 mb-2">ใบเสนอราคา {{ $quotation->document_number }}</h1>@include('Pos::partials.document-status-badge', ['status' => $quotation->status, 'label' => $statusLabels[$quotation->status] ?? $quotation->status])</div><div class="d-flex flex-wrap justify-content-end gap-2">@if(auth()->user()->hasPermission('pos.sales-quotations.print'))<a class="btn btn-app-soft d-inline-flex align-items-center gap-1" href="{{ route('pos.sales-quotations.pdf', $quotation) }}" target="_blank" rel="noopener"><i class="bx bx-printer"></i>พิมพ์ PDF</a>@endif @if($quotation->status === 'DRAFT' && auth()->user()->hasPermission('pos.sales-quotations.send'))<button type="button" class="btn btn-primary d-inline-flex align-items-center gap-1 js-quotation-send" data-url="{{ route('pos.sales-quotations.send', $quotation) }}"><i class="bx bx-send"></i>ส่งใบเสนอราคา</button>@endif @if($quotation->status === 'SENT' && auth()->user()->hasPermission('pos.sales-quotations.accept'))<button type="button" class="btn btn-primary d-inline-flex align-items-center gap-1 js-quotation-accept" data-url="{{ route('pos.sales-quotations.accept', $quotation) }}"><i class="bx bx-check-circle"></i>ตอบรับ</button>@endif @if($quotation->status === 'SENT' && auth()->user()->hasPermission('pos.sales-quotations.reject'))<button type="button" class="btn btn-outline-danger d-inline-flex align-items-center gap-1 js-quotation-reject" data-url="{{ route('pos.sales-quotations.reject', $quotation) }}" data-title="ปฏิเสธใบเสนอราคา"><i class="bx bx-x-circle"></i>ปฏิเสธ</button>@endif @if(in_array($quotation->status, ['DRAFT', 'SENT'], true) && auth()->user()->hasPermission('pos.sales-quotations.cancel'))<button type="button" class="btn btn-outline-danger d-inline-flex align-items-center gap-1 js-quotation-cancel" data-url="{{ route('pos.sales-quotations.cancel', $quotation) }}" data-title="ยกเลิกใบเสนอราคา"><i class="bx bx-x-circle"></i>ยกเลิก</button>@endif @if($quotation->order && auth()->user()->hasPermission('pos.sales-orders.view'))<a class="btn btn-app-soft d-inline-flex align-items-center gap-1" href="{{ route('pos.sales-orders.show', $quotation->order) }}"><i class="bx bx-cart"></i>ดูใบสั่งขาย</a>@elseif(in_array($quotation->status, ['SENT', 'ACCEPTED'], true) && auth()->user()->hasPermission('pos.sales-orders.create'))<form method="post" action="{{ route('pos.sales-orders.from-quotation', $quotation) }}" class="d-inline-flex">@csrf<button class="btn btn-primary d-inline-flex align-items-center gap-1" type="submit"><i class="bx bx-cart-add"></i>สร้างใบสั่งขาย</button></form>@endif @if($quotation->rfq)<a class="btn btn-outline-secondary d-inline-flex align-items-center gap-1" href="{{ route('pos.sales-rfqs.show', $quotation->rfq) }}"><i class="bx bx-arrow-back"></i>กลับไป RFQ</a>@endif</div></div>
