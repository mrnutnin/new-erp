...
<div class="container-fluid px-3 px-lg-4 py-4">
    ...
    <div class="d-flex flex-wrap justify-content-between align-items-start gap-3 mb-4">
        ...
        <div class="d-flex flex-wrap justify-content-end gap-2">
            <a class="btn btn-app-soft d-inline-flex align-items-center gap-1" href="{{ route('pos.sales-quotations.index') }}"><i class="bx bx-arrow-back"></i>กลับหน้ารายการ</a>
            ...
            @if($quotation->rfq)<a class="btn btn-app-soft d-inline-flex align-items-center gap-1" href="{{ route('pos.sales-rfqs.show', $quotation->rfq) }}"><i class="bx bx-link-external"></i>ดู RFQ ต้นทาง</a>@endif
            @if($quotation->order && auth()->user()->hasPermission('pos.sales-orders.view'))<a class="btn btn-app-soft d-inline-flex align-items-center gap-1" href="{{ route('pos.sales-orders.show', $quotation->order) }}"><i class="bx bx-cart"></i>ดูใบสั่งขาย</a>@elseif($quotation->status === 'ACCEPTED' && auth()->user()->hasPermission('pos.sales-orders.create'))<form method="post" action="{{ route('pos.sales-orders.from-quotation', $quotation) }}" class="d-inline-flex">@csrf<button class="btn btn-app-primary d-inline-flex align-items-center gap-1" type="submit"><i class="bx bx-cart-add"></i>สร้างใบสั่งขาย</button></form>@endif
            @if($quotation->status === 'DRAFT' && auth()->user()->hasPermission('pos.sales-quotations.send'))<button type="button" class="btn btn-app-primary d-inline-flex align-items-center gap-1 js-quotation-send" data-url="{{ route('pos.sales-quotations.send', $quotation) }}"><i class="bx bx-send"></i>ส่งใบเสนอราคา</button>@endif
            @if($quotation->status === 'SENT' && auth()->user()->hasPermission('pos.sales-quotations.accept'))<button type="button" class="btn btn-app-primary d-inline-flex align-items-center gap-1 js-quotation-accept" data-url="{{ route('pos.sales-quotations.accept', $quotation) }}"><i class="bx bx-check-circle"></i>ตอบรับใบเสนอราคา</button>@endif
            @if($quotation->status === 'SENT' && auth()->user()->hasPermission('pos.sales-quotations.reject'))<button type="button" class="btn btn-app-danger d-inline-flex align-items-center gap-1 js-quotation-reject" data-url="{{ route('pos.sales-quotations.reject', $quotation) }}" data-title="ปฏิเสธใบเสนอราคา"><i class="bx bx-x-circle"></i>ปฏิเสธใบเสนอราคา</button>@endif
            @if((in_array($quotation->status, ['DRAFT', 'SENT'], true) || ($quotation->status === 'ACCEPTED' && (! $quotation->order || $quotation->order->status === 'CANCELLED'))) && auth()->user()->hasPermission('pos.sales-quotations.cancel'))<button type="button" class="btn btn-app-danger d-inline-flex align-items-center gap-1 js-quotation-cancel" data-url="{{ route('pos.sales-quotations.cancel', $quotation) }}" data-title="ยกเลิกเอกสารใบเสนอราคา"><i class="bx bx-x-circle"></i>ยกเลิกเอกสาร</button>@endif
            @include('Pos::partials.draft-delete-button', ['document' => $quotation, 'permission' => 'pos.sales-quotations.delete', 'url' => route('pos.sales-quotations.destroy', $quotation)])
        </div>
    </div>
    @if($quotation->status === 'DRAFT')<div class="alert alert-info border-0 shadow-sm mb-4"><strong>ขั้นถัดไป:</strong> ตรวจสอบราคาและกดส่งใบเสนอราคาให้ลูกค้าพิจารณา</div>@elseif($quotation->status === 'SENT')<div class="alert alert-info border-0 shadow-sm mb-4"><strong>ขั้นถัดไป:</strong> บันทึกผลจากลูกค้าเป็นตอบรับหรือปฏิเสธใบเสนอราคา</div>@elseif($quotation->status === 'ACCEPTED' && ! $quotation->order)<div class="alert alert-info border-0 shadow-sm mb-4"><strong>ขั้นถัดไป:</strong> สร้างใบสั่งขายจากใบเสนอราคาที่ตอบรับแล้ว</div>@elseif($quotation->order)<div class="alert alert-success border-0 shadow-sm mb-4"><strong>มีใบสั่งขายที่เชื่อมโยงแล้ว</strong> เปิดใบสั่งขายเพื่อตรวจสอบขั้นตอนดำเนินการต่อ</div>@endif
