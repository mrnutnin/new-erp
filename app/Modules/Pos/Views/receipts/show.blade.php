...
<div class="container-fluid py-4">
    <div class="d-flex flex-wrap justify-content-between align-items-start gap-3 mb-4">
        ...
        <div class="d-flex flex-wrap align-items-center gap-2">
            ...
            <a class="btn btn-app-soft" href="{{ route('pos.receipts.index') }}"><i class="bx bx-arrow-back me-1"></i>กลับหน้ารายการ</a>
            @if (auth()->user()->hasPermission('pos.receipts.print'))
                <a class="btn btn-app-soft" href="{{ route('pos.receipts.pdf', $receipt) }}" target="_blank" rel="noopener"><i class="bx bx-printer me-1" aria-hidden="true"></i>พิมพ์ PDF</a>
            @endif
            ...
            @if ($receipt->status === 'APPROVED' && auth()->user()->hasPermission('pos.receipts.post'))
                <button class="btn btn-app-primary js-receipt-action" data-method="POST" data-url="{{ route('pos.receipts.post', $receipt) }}" @disabled(!($postReadiness['ready'] ?? true))>ยืนยันรับชำระและลงบัญชี</button>
            @endif
            @if ($receipt->status === 'POSTED' && auth()->user()->hasPermission('pos.receipts.reverse'))
                <button class="btn btn-app-danger js-receipt-reverse" data-url="{{ route('pos.receipts.reverse', $receipt) }}" data-document="{{ $receipt->document_number }}">ยกเลิกเอกสาร</button>
            @endif
            @include('Pos::partials.draft-delete-button', ['document' => $receipt, 'permission' => 'pos.receipts.delete', 'url' => route('pos.receipts.destroy', $receipt)])
        </div>
    </div>
    ...
    @if($receipt->status === 'DRAFT')<div class="alert alert-info border-0 shadow-sm mb-4"><strong>ขั้นถัดไป:</strong> อนุมัติเอกสารรับชำระก่อนยืนยันลงบัญชี</div>@elseif($receipt->status === 'APPROVED' && ($postReadiness['ready'] ?? true))<div class="alert alert-info border-0 shadow-sm mb-4"><strong>ขั้นถัดไป:</strong> ยืนยันรับชำระและลงบัญชี</div>@elseif($receipt->status === 'POSTED')<div class="alert alert-success border-0 shadow-sm mb-4"><strong>เอกสารลงบัญชีแล้ว</strong> สามารถตรวจสอบ GL หรือ Statement ลูกหนี้ที่เกี่ยวข้องได้</div>@endif
