...
<div class="container-fluid px-3 px-lg-4 py-4">
    <div class="d-flex justify-content-between align-items-end mb-4">
        ...
        @if (auth()->user()->hasPermission('pos.price-lists.create'))
            <a class="btn btn-app-primary" href="{{ route('pos.price-lists.create') }}"><i class="bx bx-plus me-1" aria-hidden="true"></i>เพิ่มรายการราคา</a>
        @endif
    </div>
    <div class="card border-0 shadow-sm mb-4"><div class="card-body p-3 p-lg-4"><div class="d-flex flex-wrap justify-content-between align-items-center gap-2 mb-3"><div><h2 class="h5 mb-1">ตัวกรองรายการราคา</h2><p class="text-secondary mb-0 small">กรองตามกลุ่มลูกค้าและสถานะก่อนค้นหาจากตาราง</p></div><button id="clear-price-list-filter" class="btn btn-sm btn-app-soft" type="button"><i class="bx bx-reset me-1" aria-hidden="true"></i>ล้างตัวกรอง</button></div><form id="price-list-filter" class="row g-3 align-items-end"><div class="col-md-4"><label class="form-label" for="price-list-group">กลุ่มลูกค้า</label><select id="price-list-group" class="form-select js-price-group" data-url="{{ route('pos.price-lists.group-options') }}"><option value="">ทุกกลุ่ม</option></select></div><div class="col-md-3"><label class="form-label" for="price-list-active">สถานะ</label><select id="price-list-active" class="form-select"><option value="">ทั้งหมด</option><option value="1">ใช้งาน</option><option value="0">ปิดใช้งาน</option></select></div><div class="col-auto"><button class="btn btn-app-primary" type="submit">ใช้ตัวกรอง</button></div></form></div></div>
    <div class="card border-0 shadow-sm"><div class="card-body p-3 p-lg-4">
        <div class="table-responsive"><table id="price-lists-table" class="table table-hover align-middle w-100" data-url="{{ route('pos.price-lists.data') }}">
            <thead><tr><th>ลำดับ</th><th>รหัส</th><th>ชื่อรายการราคา</th><th>กลุ่มลูกค้า</th><th>สกุลเงิน</th><th>รายการ</th><th>วันที่เริ่ม</th><th>วันที่สิ้นสุด</th><th>ลำดับความสำคัญ</th><th>สถานะ</th><th class="text-end">จัดการ</th></tr></thead>
        </table></div>
    </div></div>
</div>
@endsection
