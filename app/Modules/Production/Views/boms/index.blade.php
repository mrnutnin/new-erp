...
$(function(){
    ...
    $(document).on('click','.js-bom-action',function(){var button=$(this);Swal.fire({icon:'warning',text:button.data('message'),showCancelButton:true,confirmButtonText:'ดำเนินการ',cancelButtonText:'ยกเลิก'}).then(function(result){if(!result.isConfirmed)return;button.prop('disabled',true);$.post(button.data('url'),{_token:$('meta[name="csrf-token"]').attr('content'),_method:button.data('method')||'POST'}).done(function(){dt.ajax.reload(null,false);}).fail(function(xhr){Swal.fire({icon:'error',text:xhr.responseJSON?.message||'ทำรายการไม่สำเร็จ'});button.prop('disabled',false);});});});
});
</script>
@endpush
