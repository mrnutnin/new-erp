            ...
            return self::decorate([
                ...
                [
                    ...
                    'steps' => [
                        ...
                        ['label' => 'Petty Cash', 'route' => null, 'permission' => 'finance.settlements.view', 'effect' => 'วงเงิน เติมเงิน เบิกจ่าย เคลียร์ และกระทบยอดเงินสดย่อย', 'mode' => 'daily', 'block_reason' => 'Petty Cash ยังไม่มี source document และ posting contract ใน MVP จึงไม่มีปุ่มสร้างรายการ', 'recovery_hint' => 'ใช้บัญชีเงินสด/ธนาคารที่มีอยู่สำหรับรายการที่รองรับ; ห้ามสร้าง Journal แทน Petty Cash โดยไม่มีเอกสารอ้างอิง'],
                        ['label' => 'เงินทดรองพนักงาน', 'route' => null, 'permission' => 'finance.settlements.view', 'effect' => 'เบิกเงินทดรอง เคลียร์ค่าใช้จ่าย คืนเงิน หรือจ่ายเพิ่ม พร้อม audit', 'mode' => 'daily', 'block_reason' => 'Employee Advance ยังรอ subledger, settlement และ GL mapping ที่ครบถ้วน', 'recovery_hint' => 'อย่าบันทึกเป็นค่าใช้จ่ายทันทีหากยังไม่เคลียร์; รอเมนูเงินทดรองที่รองรับยอดคงเหลือและเอกสารคืนเงิน'],
            ...
            return self::decorate([
                ...
                [
                    ...
                    'decision_cards' => [
                        ...
                        ['code' => 'purchase-three-way-match', 'mode' => 'daily', 'title' => 'ตรวจ 3-way match ก่อน Post', 'description' => 'จับคู่ PO, Goods Receipt และ Credit Purchase ตาม Supplier, Warehouse, สินค้า/หน่วย, จำนวน และราคา/ต้นทุน; ตอนนี้เป็น read-only preflight ยังไม่เปิด variance approval หรือ Inventory Post จาก Receipt', 'route' => null, 'permission' => 'wms.purchase-documents.view', 'block_reason' => 'ถ้าจำนวนหรือมูลค่าไม่ตรง ให้หยุด Post และแก้เอกสารต้นทาง/จัดสรร receipt line ให้ครบก่อน; ห้ามใช้ Journal ปรับเพื่อบังคับให้ผ่าน', 'recovery_hint' => 'จำนวนต่าง: กลับไปตรวจ PO/Receipt และยอดรับคงเหลือ; ราคา/ต้นทุนต่าง: ตรวจใบเสนอราคาและใบซื้อเชื่อ แล้วส่งตาม policy variance. Draft แก้ได้, Approved Receipt ให้ Void พร้อมเหตุผล, Posted ห้ามแก้ทับให้ใช้ Reverse/Credit Note ตามสิทธิ์'],
                    ],
                    'steps' => [
                ...
                [
                    ...
                    'decision_cards' => [
                        ['code' => 'inventory-gl-mapping', 'mode' => 'setup', 'title' => '1. Inventory / COGS Mapping', 'description' => 'บัญชี Inventory และ COGS ต้องเป็นบัญชีที่ active/postable และ mapping ถูกประเภทก่อนเปิด posting', 'route' => null, 'permission' => 'wms.stock-valuation.view', 'block_reason' => 'ไปตรวจ Account Mapping ใน Accounting ให้ครบก่อน', 'recovery_hint' => 'ถ้า mapping ผิด ให้แก้ mapping แล้วทำ preflight ใหม่; ห้ามเลือกบัญชีแรกหรือ hard-code account ID'],
                        ['code' => 'inventory-cost-policy', 'mode' => 'setup', 'title' => '2. AVG / FIFO Cost Policy', 'description' => 'บริษัทต้องใช้ costing policy เดียว และกำหนด negative stock/fallback ตามนโยบาย', 'route' => null, 'permission' => 'wms.stock-valuation.view', 'block_reason' => 'ตรวจ Global Settings และยืนยัน policy ก่อน Post Stock', 'recovery_hint' => 'ถ้าเลือก policy ผิด ให้หยุด Post และแก้ setting ตาม effective-date policy ก่อนเริ่มรายการใหม่'],
                        ['code' => 'inventory-cost-preflight', 'mode' => 'daily', 'title' => '3. Pending / Unlinked Preflight', 'description' => 'ห้ามส่งต่อ Inventory→GL เมื่อยังมี pending cost หรือ allocation ที่ยังไม่ link', 'route' => 'wms.stock-valuation.index', 'permission' => 'wms.stock-valuation.view', 'block_reason' => 'เปิด Stock Valuation เพื่อตรวจ Pending/RECOST และ allocation linkage', 'recovery_hint' => 'ตรวจเอกสารรับต้นทุนและรอ RECOST ให้เสร็จ; อย่าปิดงวดหรือยืนยัน GL จากยอด provisional'],
                        ['code' => 'inventory-gl-reconciliation', 'mode' => 'daily', 'title' => '4. Inventory / COGS Reconciliation', 'description' => 'ยอด allocation, Stock projection และ GL control account ต้อง reconcile ก่อนปิดงวด', 'route' => null, 'permission' => 'wms.stock-valuation.view', 'block_reason' => 'ตรวจรายงาน reconciliation ใน Accounting เมื่อ source contract พร้อม', 'recovery_hint' => 'แยก mapping gap, pending recost, unlinked allocation และ rounding delta ก่อนแก้หรือ reverse'],
                        ['code' => 'inventory_purchase_event_wiring', 'mode' => 'setup', 'title' => 'Gate A · Purchase event wiring', 'description' => 'ตรวจว่า Receipt/Purchase source เชื่อม inventory event, Item และ UOM ครบก่อนสร้าง Cost Layer', 'route' => null, 'permission' => 'wms.stock-valuation.view', 'block_reason' => 'ยังไม่เปิด Inventory Posting จน source contract และ purchase event wiring พร้อม', 'recovery_hint' => 'ผู้รับผิดชอบ Purchasing/WMS ตรวจ source document, Item, UOM และ source identity; แก้ที่เอกสารต้นทางก่อนขอ preflight ใหม่'],
                        ['code' => 'atomic_journal_movement_allocation_linkage', 'mode' => 'daily', 'title' => 'Gate B · Atomic Journal linkage', 'description' => 'ตรวจ Movement → Allocation → Journal line linkage ให้อยู่ใน transaction เดียวและทำซ้ำไม่ได้', 'route' => null, 'permission' => 'wms.stock-valuation.view', 'block_reason' => 'ยังไม่มีปุ่ม Post เพราะ atomic movement/allocation/Journal linkage ยังไม่ผ่าน gate', 'recovery_hint' => 'ผู้รับผิดชอบ WMS/Accounting ตรวจ unlinked หรือ mismatched allocation และรอ source/posting contract; ห้าม link หรือแก้ Journal ด้วยมือ'],
                        ['code' => 'reconciliation_zero_gate', 'mode' => 'daily', 'title' => 'Gate C · Reconciliation zero', 'description' => 'ผลต่าง allocation กับ GL และ balance กับ allocation ต้องเป็นศูนย์ก่อนเปิด Posting', 'route' => null, 'permission' => 'wms.stock-valuation.view', 'block_reason' => 'ยังไม่พร้อมเปิด Inventory Posting จน reconciliation difference เป็นศูนย์และ reversal gate ผ่าน', 'recovery_hint' => 'ผู้รับผิดชอบ Accounting/WMS แยก mapping gap, pending recost, rounding และ reversal ก่อน reconcile ใหม่; ห้ามปรับยอดเพื่อบังคับให้เป็นศูนย์'],
                    ],
                    'steps' => [
                        ...
                        ['label' => 'นโยบายต้นทุน AVG/FIFO', 'route' => null, 'permission' => 'wms.stock-valuation.view', 'effect' => 'กำหนดวิธีต้นทุนระดับบริษัทก่อน Post Stock', 'block_reason' => 'ตรวจ Global Settings ให้เลือก AVG หรือ FIFO และเปิด negative stock ตามนโยบายก่อนเริ่มงาน'],
                        ['label' => 'Inventory / COGS GL Mapping', 'route' => null, 'permission' => 'wms.stock-valuation.view', 'effect' => 'เตรียมบัญชีควบคุม Inventory และ COGS ก่อนเปิด posting', 'mode' => 'setup', 'block_reason' => 'ตั้งค่าและตรวจ Account Mapping ใน Accounting ให้ครบก่อนเปิด Inventory→GL; ยังไม่มีหน้าตั้งค่าข้าม Module ใน WMS'],
                        ['label' => 'Opening Balance', 'route' => null, 'permission' => 'wms.stock-valuation.view', 'effect' => 'เตรียมยอดยกมาและต้นทุนตั้งต้นของสินค้า', 'mode' => 'setup', 'block_reason' => 'เตรียมยอดสินค้าและต้นทุนยกมาผ่าน Stock Ledger/Import ก่อนเริ่มรับ–จ่ายจริง', 'recovery_hint' => 'ตรวจยอดและหน่วยนับให้ตรงก่อนยืนยัน; หากพบความผิดพลาดหลัง Post ให้ใช้เอกสารปรับปรุงหรือ reversal ห้ามแก้ทับ ledger เดิม'],
                        ['label' => 'Receipt / Issue / Transfer', 'route' => 'wms.stock.index', 'permission' => 'wms.stock.view', 'effect' => 'บันทึกความเคลื่อนไหวและสร้าง cost allocation ตาม AVG/FIFO', 'mode' => 'daily', 'recovery_hint' => 'ตรวจคลัง สินค้า หน่วย และวันที่ก่อน Post; หาก Post ผิดให้ใช้ reversal/เอกสารแก้ไขตามสถานะ ห้ามลบหรือแก้ทับ stock ledger เดิม'],
                        ['label' => 'Count / Adjust', 'route' => null, 'permission' => 'wms.stock.view', 'effect' => 'ตรวจนับและปรับปรุงส่วนต่างอย่างมีเหตุผลตาม approval policy', 'mode' => 'daily', 'block_reason' => 'สร้าง Stock Count/Adjustment ตามสิทธิ์และ approval policy; บริษัททีมเล็กไม่ต้องสร้างผู้อนุมัติคนที่สอง', 'recovery_hint' => 'ถ้านับผิดให้แก้ Draft ก่อน Post; หลัง Post ให้ทำ adjustment/reversal พร้อมเหตุผลและ audit ห้ามแก้ยอดเดิมโดยตรง'],
                        ['label' => 'Inventory→GL Preflight', 'route' => null, 'permission' => 'wms.stock-valuation.view', 'effect' => 'ตรวจ mapping, pending cost, allocation linkage และ period ก่อน posting', 'mode' => 'daily', 'block_reason' => 'ตรวจ preflight ให้ไม่มี mapping ที่ขาด, pending cost หรือ allocation ที่ยังไม่ link ก่อนส่งต่อ Accounting'],
                        ...
                        ['label' => 'GL Reconciliation / Resolve', 'route' => null, 'permission' => 'wms.stock-valuation.view', 'effect' => 'ทบทวนผลต่าง Inventory/COGS กับ GL และแก้รายการค้างก่อนปิดงวด', 'mode' => 'daily', 'block_reason' => 'ใช้รายงาน reconciliation ใน Accounting เมื่อตั้งค่า mapping และรายงานข้าม Module พร้อม; WMS ยังไม่มีปุ่ม resolve ข้าม Module'],
                    ],
                ],
            ], 'daily');
        }
