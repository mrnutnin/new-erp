    ...
    public static function for(string $program, ?ModuleCapability $capability = null): array
    {
            ...
            return self::decorate([
                ...
                [
                    ...
                    'decision_cards' => [
                        ...
                        ['code' => 'purchase-three-way-match', 'mode' => 'daily', 'title' => 'ตรวจ 3-way match ก่อน Post', 'description' => 'จับคู่ PO, Goods Receipt และ Credit Purchase ตาม Supplier, Warehouse, สินค้า/หน่วย, จำนวน และราคา/ต้นทุน พร้อม snapshot และ variance approval ก่อนอนุมัติหรือ Post', 'route' => 'purchasing.purchase-documents.index', 'permission' => 'purchasing.purchase-documents.view', 'block_reason' => 'หากมี variance ให้เปิดเอกสารและอนุมัติผลต่างจาก snapshot เดียวกัน; blocker ด้าน linkage ต้องแก้ที่เอกสารต้นทางเท่านั้น และห้ามใช้ Journal ปรับเพื่อบังคับให้ผ่าน', 'recovery_hint' => 'จำนวนต่าง: ตรวจ PO/Receipt และยอดรับคงเหลือ; ราคา/ต้นทุนต่าง: ตรวจใบซื้อเชื่อแล้วส่งอนุมัติ variance. หาก snapshot เปลี่ยนต้องเริ่มตรวจใหม่; Posted ห้ามแก้ทับให้ใช้ Reverse/Credit Note ตามสิทธิ์'],
                    ],
                    'steps' => [
                ...
                [
                    ...
                    'decision_cards' => [
                        ...
                        ['code' => 'inventory_purchase_event_wiring', 'mode' => 'setup', 'title' => 'Gate A · Purchase source & inventory mapping', 'description' => 'ตรวจเอกสารต้นทาง สินค้า หน่วยนับ และบัญชีสินค้าคงเหลือให้ครบก่อนส่งข้อมูลเข้าสต็อกและบัญชี', 'route' => 'wms.stock-valuation.index', 'permission' => 'wms.stock-valuation.view', 'event_code' => 'inventory_purchase_event_wiring', 'block_reason' => 'ยังไม่พร้อม ตรวจรายการสินค้าและเอกสารต้นทางที่แสดงด้านล่าง', 'recovery_hint' => 'ตรวจรายการสินค้าและเอกสารต้นทางที่ระบุ; บัญชีสินค้าคงเหลือต้องเป็นประเภท INVENTORY ที่ใช้งานและลงบัญชีได้'],
        ...
        if ($program === 'production') {
            ...
            return self::decorate([[
                ...
                'steps' => [
                    ['label' => 'Item / BOM', 'route' => 'production.boms.index', 'permission' => 'production.boms.view', 'effect' => 'เตรียมโครงสร้างสินค้าและวัตถุดิบก่อนวางแผนผลิต', 'mode' => 'setup'],
                    ['label' => 'Sales Demand / Work Order', 'route' => 'production.demand.index', 'permission' => 'production.orders.view', 'effect' => 'ตรวจคำขอสั่งผลิตจากฝ่ายขายและสร้างใบสั่งผลิตตามแผน', 'mode' => 'daily'],
                    ['label' => 'Shop Floor / Production Execution', 'route' => 'production.shop-floor.index', 'permission' => 'production.shop_floor.use', 'effect' => 'ติดตามงานหน้างาน บันทึกการปฏิบัติงาน และจัดการปัญหาการผลิต', 'mode' => 'daily'],
                    ['label' => 'Work Order / Material Issue / Finished Goods', 'route' => 'production.orders.index', 'permission' => 'production.orders.view', 'effect' => 'จัดการใบสั่งผลิต ตรวจการเบิกวัตถุดิบ ต้นทุน WIP และรับสินค้าสำเร็จรูป', 'mode' => 'daily'],
                ],
            ]], 'setup');
        }
