...
@section('content')
    @php
        ...
        $rows = old('lines', $document->exists ? $document->lines->map(fn ($line) => [...$line->only(['description','item_id','uom_id','purchase_order_line_id','account_id','tax_code_id','quantity','unit_price','discount_amount']), 'receipt_allocations' => $line->receiptAllocations->map->only(['goods_receipt_line_id','allocated_quantity'])->all()])->all() : ($prefillLines ?? [['description'=>'','item_id'=>'','uom_id'=>'','purchase_order_line_id'=>'','account_id'=>'','tax_code_id'=>'','quantity'=>'1.0000','unit_price'=>'0.0000','discount_amount'=>'0.00','receipt_allocations'=>[]]]));
        ...
        $taxCalculation = old('tax_calculation', $document->tax_treatment === 'NONE_VAT' ? 'NONE' : ($document->prices_include_vat ? 'VAT_INCLUSIVE' : 'VAT_EXCLUSIVE'));
        $initialTaxRates = $lineTaxCodes->mapWithKeys(fn ($taxCode) => [(string) $taxCode->id => (float) $taxCode->rate])->all();
        $taxProfileLocked = $type === 'CREDIT_NOTE' && $selectedOriginal;
    @endphp
    <div class="container-fluid px-3 px-lg-4 py-4">
        ...
        <form id="purchase-document-form" class="purchase-mode-{{ strtolower($purchaseMode) }}" method="{{ $document->exists ? 'PUT' : 'POST' }}" action="{{ $document->exists ? route($moduleRoutePrefix.'.purchase-documents.update', $document) : route($moduleRoutePrefix.'.purchase-documents.store') }}">
            ...
            <div class="card border-0 shadow-sm mb-4"><div class="card-body p-4"><div class="row g-3">
                ...
                <div class="col-md-4"><label class="form-label" for="purchase-tax-calculation">การคำนวณภาษี <span class="text-danger">*</span></label><select class="form-select" id="purchase-tax-calculation" name="tax_calculation" required><option value="VAT_INCLUSIVE" @selected($taxCalculation === 'VAT_INCLUSIVE')>รวมภาษี</option><option value="VAT_EXCLUSIVE" @selected($taxCalculation === 'VAT_EXCLUSIVE')>ภาษีนอก</option><option value="NONE" @selected($taxCalculation === 'NONE')>ไม่มีภาษี</option></select><div class="form-text">กำหนดวิธีคำนวณเดียวสำหรับทั้งเอกสาร</div><div class="invalid-feedback" data-error-for="tax_calculation"></div></div>
            ...
            <div class="card border-0 shadow-sm"><div class="card-body p-4"><div class="d-flex flex-column flex-lg-row justify-content-between align-items-lg-center gap-3 mb-3"><div><h2 class="h5 mb-1">รายการตั้งหนี้</h2><span class="badge {{ $taxCalculation === 'NONE' ? 'app-status-neutral' : 'app-status-info' }}" id="tax-label">{{ ['VAT_INCLUSIVE' => 'รวมภาษี', 'VAT_EXCLUSIVE' => 'ภาษีนอก', 'NONE' => 'ไม่มีภาษี'][$taxCalculation] ?? 'ไม่มีภาษี' }}</span><span class="small text-secondary d-block mt-1" id="purchase-mode-hint"></span></div><div class="d-flex flex-wrap gap-2 align-items-center"><div class="purchase-gr-picker-wrap"><select id="purchase-gr-picker" class="form-select form-select-sm" data-url="{{ route($moduleRoutePrefix.'.purchase-documents.goods-receipt-options') }}" aria-label="เพิ่มรายการจาก Goods Receipt"><option value="">เพิ่มรายการจาก Goods Receipt</option></select></div><button class="btn btn-sm btn-app-soft js-expense-only" id="add-purchase-line" type="button"><i class="bx bx-plus me-1"></i><span>เพิ่มรายการบริการ</span></button></div></div>
                ...
                <div class="d-flex justify-content-end"><div class="text-end"><div class="small text-secondary" id="purchase-total-label">ยอดรวมทั้งสิ้น</div><div class="h4 mb-0" id="purchase-total">0.00</div></div></div>
                <div class="invalid-feedback d-block" data-error-for="lines"></div>
                <div class="mt-4"><button class="btn btn-app-primary" type="submit"><i class="bx bx-save me-1"></i>บันทึกร่าง</button><a class="btn btn-app-soft" href="{{ route($moduleRoutePrefix.'.purchase-documents.index') }}"><i class="bx bx-arrow-back me-1" aria-hidden="true"></i>กลับหน้ารายการ</a></div>
            </div></div>
        </form>
    </div>
@endsection
...
@push('scripts')
    <script>
        $(function () {
            ...
            var initialTaxRates = @json($initialTaxRates);
            var purchaseMode = @json($purchaseMode);
            var taxProfileLocked = @json((bool) $taxProfileLocked);
            var creditCreateUrl = @json(route($moduleRoutePrefix.'.purchase-documents.create', ['document_type' => 'CREDIT_NOTE']));
            ...
            @endforeach
            function lockOriginalTaxProfile() {
                if (!taxProfileLocked) return;
                var $calculation=$('#purchase-tax-calculation'), calculationName=$calculation.attr('name');
                if (!$calculation.next('.js-locked-tax-value').length) $('<input>',{type:'hidden',class:'js-locked-tax-value',name:calculationName,value:$calculation.val()}).insertAfter($calculation);
                $calculation.prop('disabled',true);
                $lines.find('.js-purchase-tax').each(function(){ var $tax=$(this), name=$tax.attr('name'); if (!$tax.next('.js-locked-tax-value').length) $('<input>',{type:'hidden',class:'js-locked-tax-value',name:name,value:$tax.val()||''}).insertAfter($tax); $tax.prop('disabled',true); });
                var $supplier=$('#purchase-supplier'), supplierName=$supplier.attr('name');
                if (!$supplier.next('.js-locked-source-value').length) $('<input>',{type:'hidden',class:'js-locked-source-value',name:supplierName,value:$supplier.val()}).insertAfter($supplier);
                $supplier.prop('disabled',true);
                var $original=$('#purchase-original'), originalName=$original.attr('name');
                if (!$original.next('.js-locked-source-value').length) $('<input>',{type:'hidden',class:'js-locked-source-value',name:originalName,value:$original.val()}).insertAfter($original);
                $original.prop('disabled',true);
                $('#add-purchase-line').hide();
            }
            ...
            function total() { var mode=$('#purchase-tax-calculation').val(), inclusive=mode==='VAT_INCLUSIVE', vat=mode!=='NONE', sum=0; $lines.find('.purchase-line').each(function () { var $row=$(this), $tax=$row.find('.js-purchase-tax'), selected=($tax.select2('data')||[])[0]||{}, rate=vat?Number(selected.rate||initialTaxRates[String($tax.val())]||0):0; var before=Math.round((parseFloat($row.find('.js-quantity').val())||0)*(parseFloat($row.find('.js-unit-price').val())||0)*100)/100; var entered=Math.max(0,before-(parseFloat($row.find('.js-discount').val())||0)), base=inclusive&&rate?entered/(1+rate/100):entered, tax=rate?(inclusive?entered-base:base*rate/100):0, gross=base+tax; $row.find('.js-line-total').text(gross.toLocaleString('th-TH',{minimumFractionDigits:2,maximumFractionDigits:2})); sum+=gross; }); $('#purchase-total').text(sum.toLocaleString('th-TH',{minimumFractionDigits:2,maximumFractionDigits:2})); }
            function togglePurchaseMode() { var inventory=purchaseMode==='INVENTORY', credit=$('[name="document_type"]').val()==='CREDIT_NOTE', nonReturn=credit && $('#purchase-credit-mode').val()==='NON_RETURN'; $('#purchase-document-form').toggleClass('purchase-mode-inventory',inventory).toggleClass('purchase-mode-expense',!inventory); $('.purchase-gr-picker-wrap').toggle(inventory && !nonReturn); $('.js-expense-only').toggle(!inventory && !taxProfileLocked); $('#purchase-gr-picker').prop('disabled', !inventory || credit || nonReturn || !$('#purchase-supplier').val()); $('#add-purchase-line span').text(inventory?'เพิ่มรายการสินค้า':'เพิ่มรายการบริการ'); $('#purchase-mode-hint').text(taxProfileLocked?'ระบบคัดลอกรูปแบบภาษีและ Tax Code จากใบตั้งหนี้ต้นทางแล้ว ไม่สามารถแก้ไขได้':(nonReturn?'Credit Note แบบไม่คืนสินค้า: ไม่ต้องเลือก Goods Receipt และไม่มีผลต่อ Stock/Cost':(inventory?'เลือก Goods Receipt เพื่อดึงสินค้า จำนวน และราคาจาก PO':'เลือกได้เฉพาะรายการประเภทบริการ และไม่ต้องอ้างอิง PO/GR'))); $lines.find('.purchase-line').each(function(){ mergeInventoryFields($(this)); }); $lines.find('.js-purchase-item').each(function(){ var $select=$(this); if ($select.hasClass('select2-hidden-accessible')) $select.select2('destroy'); initItem($select); }); }
            ...
            $('#purchase-original').on('select2:select', function(event){ window.location.assign(creditCreateUrl+'&original_document_id='+encodeURIComponent(event.params.data.id)); });
            $('#purchase-supplier').on('change',function(){ selectedGoodsReceiptIds={}; $('#purchase-original').val(null).trigger('change'); $('#purchase-gr-picker').val(null).trigger('change').prop('disabled', purchaseMode!=='INVENTORY' || !$(this).val()); }); $('#purchase-mode').on('change',function(){ var next=$(this).val(), hasData=$lines.find('.js-purchase-item').filter(function(){ return $(this).val() || $(this).closest('.purchase-line').find('.js-purchase-linkage').length; }).length; if(hasData && !window.confirm('การเปลี่ยนประเภทการซื้อจะต้องเลือกข้อมูลรายการใหม่ ต้องการเปลี่ยนหรือไม่?')){ $(this).val(purchaseMode); return; } purchaseMode=next; selectedGoodsReceiptIds={}; $lines.find('.purchase-line').each(function(){ mergeInventoryFields($(this)); }); $lines.find('.js-purchase-linkage').remove(); $lines.find('.js-purchase-item,.js-purchase-uom').val(null).trigger('change'); togglePurchaseMode(); }); $('#purchase-gr-picker').prop('disabled', purchaseMode!=='INVENTORY' || !$('#purchase-supplier').val()); $('[name="document_type"]').on('change',toggleType); $('#purchase-tax-calculation').on('change',function(){ var mode=$(this).val(), vat=mode!=='NONE', labels={VAT_INCLUSIVE:'รวมภาษี',VAT_EXCLUSIVE:'ภาษีนอก',NONE:'ไม่มีภาษี'}; $('#tax-label').text(labels[mode]||labels.NONE).toggleClass('app-status-info',vat).toggleClass('app-status-neutral',!vat); $lines.find('.js-purchase-tax').prop('disabled',!vat); if(!vat) $lines.find('.js-purchase-tax').val(null).trigger('change'); total(); });
            $('#add-purchase-line').on('click',function(){ var index=$lines.children().length; var $row=$('<tr class="purchase-line"><td><input class="form-control" name="lines['+index+'][description]" required><select class="form-select js-purchase-item mt-1" name="lines['+index+'][item_id]"><option value="">บริการ/ไม่ผูกสินค้า</option></select></td><td><input class="form-control text-end js-quantity" type="number" min="0.0001" step="0.0001" name="lines['+index+'][quantity]" value="1.0000" required><select class="form-select js-purchase-uom mt-1" name="lines['+index+'][uom_id]"><option value="">เลือกหน่วย</option></select></td><td class="js-purchase-linkage-slot"></td><td><select class="form-select js-purchase-account" name="lines['+index+'][account_id]" required></select><select class="form-select js-purchase-tax mt-1" name="lines['+index+'][tax_code_id]"><option value="">NONE</option></select></td><td><input class="form-control text-end js-unit-price" type="number" min="0" step="0.0001" name="lines['+index+'][unit_price]" value="0.0000" required><input class="form-control text-end js-discount mt-1" type="number" min="0" step="0.01" name="lines['+index+'][discount_amount]" value="0.00" required></td><td class="text-end js-line-total">0.00</td><td class="text-end text-nowrap"><button class="btn btn-sm btn-outline-danger js-remove-line" title="ลบบรรทัด" aria-label="ลบบรรทัด" type="button"><i class="bx bx-trash" aria-hidden="true"></i></button></td></tr>'); $lines.append($row); initItem($row.find('.js-purchase-item')); initUom($row.find('.js-purchase-uom')); initAccount($row.find('.js-purchase-account')); initTax($row.find('.js-purchase-tax')); $row.find('.js-purchase-tax').prop('disabled',$('#purchase-tax-calculation').val()==='NONE'); });
            $lines.on('click','.js-remove-line',function(){ if($lines.children().length>1){ $(this).closest('tr').remove(); reindex(); total(); } }); $lines.on('input change','.js-quantity,.js-unit-price,.js-discount,.js-purchase-tax',total);
            $('#purchase-credit-mode').on('change', togglePurchaseMode); toggleType(); $('#purchase-tax-calculation').trigger('change'); lockOriginalTaxProfile(); total(); window.erpAjaxForm({ form:'#purchase-document-form', redirect:true });
        });
    </script>
@endpush
